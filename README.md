...
Revealed question pages order answers by initial submission time and stable answer ID, label them anonymously as `Answer 1`, `Answer 2`, and so on, and render excerpts without embedding bodies. Each full body is fetched only when selected, and multiple bodies can remain open for comparison. Other-user answers remain unavailable to WebMCP and unauthenticated requests.
