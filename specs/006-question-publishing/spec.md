...
認証済みHumanは、Question本文、主言語、回答締切を入力し、公開前の下書きとして保存する。入力中に誤りがある場合は、どの項目を直す必要があるかを理解でき、入力済みの有効な内容を失わずに修正できる。
...
1. **前提** 認証済みHumanが作成画面を開いている、**操作** 10〜1,000文字のQuestion本文、対応する主言語、現在から1時間以上30日以内の回答締切を入力し、公開内容の確認を選択して保存する、**結果** Questionは本人所有の `DRAFT` として1件保存され、公開Questionとしては表示されない。
2. **前提** 認証済みHumanが作成画面へ有効な値を入力している、**操作** 本文、主言語、締切、または公開内容の確認を無効な値にして保存する、**結果** 保存は行われず、各該当項目の近くに英語のエラーが表示され、有効な入力値は保持される。
...
Question作成者は、自分の下書きの本文、主言語、回答締切を確認・修正し、公開を明示的に確定する。公開後はPersonal Agentが回答対象として扱える状態になり、問いの意味や回答条件が後から変わらない。
...
2. **前提** 本人所有の下書きがある、**操作** 公開前に本文、主言語、または締切を有効な値へ変更して保存する、**結果** 更新内容が同じ下書きへ反映され、公開前の確認に表示される。
3. **前提** Questionが公開済みである、**操作** 作成者が本文、主言語、締切の変更または再公開を試みる、**結果** 既存内容と公開時刻は変わらず、公開済みのため操作できないことが英語で表示される。
...
2. **前提** 他人所有の公開済みQuestionがある、**操作** 認証済みHumanが管理用の変更または公開操作を試みる、**結果** 操作は拒否され、Questionの本文、主言語、締切、公開時刻は変化しない。
...
- 主言語と本文の実際の言語が一致しないように見えても、MVPでは自動判定で上書きせず、作成者が選んだ主言語を正とする。
...
- **FR-002**: システムは、Question入力として本文、主言語、回答締切、および公開内容に機密・個人情報を含めないことの確認を必須にしなければならない。
- **FR-003**: システムは、Question本文を前後の空白を除いた10〜1,000文字に制限し、空白だけの本文、制御目的だけの文字列、範囲外の本文を保存してはならない。
- **FR-004**: システムは、Questionの主言語をMVPで対応する `English` または `Japanese` から作成者が1つ選ぶ方式とし、保存値をそれぞれ `en` または `ja` として一意に扱わなければならない。
- **FR-005**: システムは、主言語の自動推定結果を保存条件にせず、作成者が選択した値をQuestionとPersonal Agent回答言語の基準として保持しなければならない。
...
- **FR-010**: システムは、本人所有の `DRAFT` に限って本文、主言語、回答締切、および公開内容の確認を更新できるようにしなければならない。
- **FR-011**: システムは、公開前に本文、主言語、回答締切、公開後は編集できないこと、および回答が締切までsealedであることを示す確認画面を提供しなければならない。
...
- **FR-014**: システムは、公開済みQuestionの本文、主言語、回答締切、Reveal時刻、作成者を、画面および直接の管理操作のいずれからも変更できないようにしなければならない。
...
- **FR-024**: システムは、報告、管理者審査、Question削除、自動翻訳、自動言語判定を本SPECで提供してはならない。
- **FR-025**: システムは、Question作成・公開機能から他者のAnswer本文、Excerpt、投稿者識別情報を返してはならず、`My Questions` では集計された回答数だけを扱わなければならない。

### 主要エンティティ

- **Question**: 認証済みHumanが作成する問い。作成者、本文、主言語、公開時刻、回答締切、Reveal時刻、作成・更新時刻を持ち、公開時刻の有無と時刻境界から現在状態が決まる。
- **Question Draft Input**: 下書き作成・編集時にHumanが提示する本文、主言語、ローカル回答締切とタイムゾーン、公開内容の確認。Questionとして確定する前にすべての入力規則を満たす必要がある。
...
- **SC-002**: 本文の下限・上限、対応言語、締切の両境界、確認有無を含む30件以上の入力検証ケースで、期待する受理・拒否結果との一致率が100%となる。
...
- **SC-007**: 公開前確認を行った検証参加者の100%が、Questionの主言語、回答締切、公開後に編集できないこと、Answerが締切までsealedであることを公開前に正しく説明できる。
...
- MVPでQuestionの主言語として提供するのは、SPEC 003で安全性と言語一致をCritical Go検証済みの英語と日本語である。他言語は後続拡張とする。
...
- 画面上の表示文言、ボタン名、項目ラベル、エラーメッセージは英語とし、Question本文自体は選択した主言語で入力する。
...
- SPEC 003「Personal Agent回答の安全性・言語の検証」が完了し、英語・日本語のQuestionに対する同一言語回答方針を利用できること。
