...
`instructions` は、利用可能なUser Contextの参照元3種と、Context根拠、Private Context、未信頼Question、投稿許可、投稿確認に関する固定booleanを返す。正確なFieldは [WebMCP 5 Tool契約](./contracts/webmcp-tools_ja.md) を正本とする。回答言語を指定するメタデータは返さず、時刻はタイムゾーンに依存しない絶対時刻として返す。
