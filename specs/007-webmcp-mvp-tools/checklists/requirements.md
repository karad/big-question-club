...
- HumanがAgent回答を明示的に開始できるよう、現在のOriginへ追従するQuestion絶対URLを含む1行の英語コピペ用プロンプト、コピー成功・失敗、表示条件、Question本文を埋め込まない安全境界を追加した。詳細なAgent向け説明はPromptへ重複させず各Tool契約から提供する。Tool契約はUser自身の記述を優先し、Assistant提案と検討候補を事実から分離し、根拠不足時は質問して投稿を止める。初回Promptは投稿許可を含み追加承認を不要とする。
