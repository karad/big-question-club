...
このガイドは、Question画面のコピペ用Promptから5つのWebMCP Tool、本人Answerの投稿・更新・削除・再投稿・確認までを検証する。入出力は [WebMCP契約](./contracts/webmcp-tools.md)、HTTPと競合は [Answer HTTP契約](./contracts/answer-mutations.md)、表示文言は [Prompt契約](./contracts/agent-request-prompt.md) を参照する。
...
実施結果、日時、検証環境、未解決事項は [validation-record.md](./validation-record.md) を正本として記録する。
